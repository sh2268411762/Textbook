...
//	printf("判断实参是否为字母（大写：A 到 Z 或 小写 a 到 z）\n\n");
//
...
//		"L is coverted to lowercase is ", tolower('L'));
...
//		"Newline（换行符）", isspace('\n') ? " is a " : " is not a ", "whitespace character", "Horizontal tab（水平制表符）",
...
//		"Alert（警告）", isprint('\a') ? " is a " : " is not a ", "printing character");
//	printf("判断是否是一个可以打印在屏幕上的字符\n\n");
//
//	printf("%s\n%s%s\n%s%s%s\n", "According to isgraph：",
//		isgraph('Q') ? "Q is a " : "Q is not a ", "printing character other than space",
//		"Space（空格）", isgraph(' ') ? " is a " : " is not a ", "printing character other than space");
